...
3. **임베딩** -- Ollama(`mxbai-embed-large`)로 벡터 임베딩 생성
4. **벡터 저장** -- SQLite + `sqlite-vec` 확장에 임베딩 저장
5. **검색** -- 쿼리를 HyDE 변환(가상 코드 생성) 후 인코딩하고 코사인 유사도로 가장 가까운 벡터를 검색
...
> **참고:** 기존에 `nomic-embed-text`로 인덱싱한 프로젝트는 모델 변경 후 `localgrep index --full`로 재인덱싱이 필요합니다. 임베딩 차원이 768에서 1024로 변경되어 기존 인덱스와 호환되지 않습니다.

---

## 기술 결정

### 왜 mxbai-embed-large인가?

4개 임베딩 모델을 15개 코드 검색 쿼리로 비교 테스트했습니다:

| 모델 | 차원 | Hit@1 | Hit@3 | Hit@5 | 크기 |
|------|------|-------|-------|-------|------|
| mxbai-embed-large | 1024 | **73%** | **87%** | **100%** | 670MB |
| gte-qwen2-1.5b | 1536 | 67% | 73% | 100% | 3.6GB |
| nomic-embed-text | 768 | 47% | 73% | 80% | 274MB |
| snowflake-arctic-embed | 1024 | 33% | 33% | 40% | 669MB |

mxbai-embed-large가 크기 대비 가장 높은 정확도를 보였습니다.

### 왜 Pure Vector인가?

3가지 검색 알고리즘을 비교했습니다:

| 알고리즘 | Hit@1 | Hit@3 | Hit@5 |
|---------|-------|-------|-------|
| Pure Vector (코사인) | **73%** | **87%** | **100%** |
| Hybrid (벡터+BM25) | 73% | 87% | 87% |
| Vector + Rerank | 67% | 87% | 93% |

mxbai-embed-large가 충분히 강력해서 BM25 하이브리드나 리랭킹이 오히려 노이즈를 추가했습니다.

### 왜 HyDE인가?

6가지 임베딩 전략을 비교했습니다:

| 전략 | Hit@1 | Hit@3 | Hit@5 |
|------|-------|-------|-------|
| HyDE (가상 코드 쿼리) | 73% | **100%** | **100%** |
| AST + Context | **87%** | 87% | 87% |
| Baseline (현재) | 73% | 87% | 100% |
| AST 청킹 | 73% | 87% | 100% |
| Hierarchical | 80% | 87% | 87% |
| Context-enriched | 73% | 73% | 73% |

**HyDE가 유일하게 Hit@3 100%를 달성했습니다.**

자연어 쿼리와 코드는 임베딩 공간에서 거리가 멀지만,
쿼리를 "이런 코드가 있을 것이다"라는 가상 코드로 변환하면
실제 코드와 임베딩 공간에서 더 가까워집니다.

컨텍스트 강화(import/시그니처 추가)는 오히려 노이즈가 되어 검색 품질을 떨어뜨렸습니다.
